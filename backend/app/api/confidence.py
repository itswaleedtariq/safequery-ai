...
@router.post(
    "/confidence/check",
    response_model=ConfidenceCheckResponse,
    summary="Calculate explainable SQL confidence",
)
def calculate_sql_confidence(
    request: ConfidenceCheckRequest,
) -> ConfidenceCheckResponse:
    """Validate SQL and calculate a confidence report."""

    try:
        return check_confidence(request)
