...
def _get_categorical_samples(
    ...
    High-cardinality columns such as customer email addresses are not
    included because they would make the schema unnecessarily large.
    """
    ...
    statement = (
        ...
        .distinct()
        .limit(settings.schema_categorical_max_distinct + 1)
    )

    values = list(connection.execute(statement).scalars().all())

    if len(values) > settings.schema_categorical_max_distinct:
        return []

    return values[: settings.schema_sample_limit]
...
def _build_column_schema(
    ...
    default_value = column_information.get("default")

    computed_information = column_information.get("computed")
    computed_expression: str | None = None

    if computed_information:
        computed_sql = computed_information.get("sqltext")

        if computed_sql is not None:
            computed_expression = str(computed_sql)

    return ColumnSchema(
        name=column_name,
        data_type=str(column_type),
        nullable=bool(column_information.get("nullable", True)),
        default=str(default_value) if default_value is not None else None,
        primary_key=column_name in primary_key_columns,
        comment=column_information.get("comment"),
...
def _build_foreign_key_schema(
    ...
    options = foreign_key_information.get("options") or {}

    return ForeignKeySchema(
        name=foreign_key_information.get("name"),
        constrained_columns=(
            foreign_key_information.get("constrained_columns") or []
        ),
        referred_schema=foreign_key_information.get("referred_schema"),
        referred_table=foreign_key_information["referred_table"],
        referred_columns=(
            foreign_key_information.get("referred_columns") or []
        ),
...
def _build_index_schema(
    ...
    column_names = [
        column_name
        for column_name in (index_information.get("column_names") or [])
        if column_name is not None
    ]

    return IndexSchema(
        name=index_information.get("name"),
        column_names=column_names,
        unique=bool(index_information.get("unique", False)),
    )


@lru_cache(maxsize=1)
def introspect_database_schema() -> DatabaseSchemaResponse:
    """
    Inspect PostgreSQL and return tables, columns and relationships.

    The result is cached because the database structure normally does
    ...
    with engine.connect() as connection:
        ...
        for table_name in table_names:
            primary_key_information = inspector.get_pk_constraint(
                table_name,
                schema=schema_name,
            )

            primary_key_columns = set(
                primary_key_information.get("constrained_columns") or []
            )
            ...
            foreign_key_information = inspector.get_foreign_keys(
                table_name,
                schema=schema_name,
            )

            foreign_keys = [
                _build_foreign_key_schema(foreign_key)
                for foreign_key in foreign_key_information
            ]

            index_information = inspector.get_indexes(
                table_name,
                schema=schema_name,
            )
